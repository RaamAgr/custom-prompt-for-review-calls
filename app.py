# app.py — FINAL VERSION (DOT NOTATION - NO INDENTATION ERRORS)
# -----------------------------------------------------------------------------
# 1. Uses 'st.sidebar.text_input' instead of 'with st.sidebar:'.
# 2. This guarantees the Prompt Editor stays on the Main Page.
# -----------------------------------------------------------------------------
...
def main():
    ...
    # --- SIDEBAR: STRICTLY SETTINGS ---
    # Using dot notation guarantees this stays in sidebar
    st.sidebar.header("⚙️ Settings")
    api_key = st.sidebar.text_input("Gemini API Key", type="password")
    max_workers = st.sidebar.slider("Concurrency", 1, 128, 4)
    keep_remote = st.sidebar.checkbox("Keep Google Files", False)
    st.sidebar.divider()
    language_mode = st.sidebar.selectbox("Language Context", ["English", "Hindi", "Hinglish"], index=2)
    st.sidebar.divider()
    theme_choice = st.sidebar.radio("UI Theme", ["Light", "Dark"], 0, horizontal=True)

    # --- MAIN PAGE: PROMPT EDITOR ---
    # Since we are NOT in a 'with st.sidebar' block, this goes to Main.
